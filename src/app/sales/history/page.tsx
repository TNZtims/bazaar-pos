    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-100">Sales History</h1>
            <p className="mt-1 text-sm text-slate-400">Complete transaction history and analytics</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-slate-800 rounded-lg shadow-sm border border-slate-700 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-slate-100">{sales.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-800 rounded-lg shadow-sm border border-slate-700 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <h3 className="text-sm font-medium text-slate-400">Avg Order Value</h3>
                <p className="text-2xl font-semibold text-slate-100">{formatCurrency(averageOrderValue)}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          {sales.length === 0 && !loading ? (
            ...
            <>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-700">
                  ...
                  <tbody className="bg-slate-800 divide-y divide-slate-700">
                    {sales.map((sale) => (
                      <tr key={sale._id} className="hover:bg-slate-700 transition-colors">
              ...
              <div className="md:hidden space-y-4 p-4">
                {sales.map((sale) => (
                  <div key={sale._id} className="border border-gray-200 dark:border-slate-600 rounded-lg p-4 bg-slate-700">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <p className="font-semibold text-slate-100">{formatCurrency(sale.finalAmount)}</p>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1 flex justify-between sm:hidden">
                      <button
                        ...
                        className="relative inline-flex items-center px-4 py-2 border border-slate-600 text-sm font-medium rounded-md text-slate-300 bg-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                      >
                        Previous
                      </button>
                      <button
                        ...
                        className="ml-3 relative inline-flex items-center px-4 py-2 border border-slate-600 text-sm font-medium rounded-md text-slate-300 bg-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                      >
                        Next
                      </button>
                    </div>
                      ...
                      <div>
                        <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                          <button
                            onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
                            disabled={currentPage === 1}
                            className="relative inline-flex items-center px-2 py-2 rounded-l-md border border-slate-600 bg-slate-700 text-sm font-medium text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                          >
                            Previous
                          </button>
                          <button
                            onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))}
                            disabled={currentPage === totalPages}
                            className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-slate-600 bg-slate-700 text-sm font-medium text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                          >
                            Next
                          </button>
                        </nav>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-slate-100">{formatDate(selectedSale?.createdAt)}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-300">Customer</label>
                      <p className="text-sm text-slate-100">{selectedSale?.customerName || 'Walk-in Customer'}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-300">Sale ID</label>
                      <p className="text-sm text-slate-100 font-mono">{selectedSale?._id?.slice(-8)}</p>
                    </div>
                  </div>

                  {/* Items */}
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">Items Purchased</label>
                  ...
                  {selectedSale?.notes && (
                    <div>
                      ...
                      <p className="text-sm text-slate-100 bg-gray-50 dark:bg-slate-700 p-3 rounded-md">{selectedSale?.notes}</p>
                    </div>
                  )}
                </div>
